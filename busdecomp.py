...
def busdecomp_gtfs(base_path, comp_path, road_path, 
                   gtfs_shapes = False, compare = True, metrics = False, port = 8002):
    
    # Generate the initial shapes defining the path of the bus routes.
    if gtfs_shapes:
        base_segments = shape_matching(base_path)
        comp_segments = shape_matching(comp_path)
    else: 
        base_segments = map_matching(base_path)
        comp_segments = map_matching(comp_path)
